...
export default function PIDDemo({
	...
	const simulate = useCallback(
		(currentState: MotorState, target: number, kpVal: number, kdVal: number, kiVal: number, massVal: number, noise: boolean) => {
			const measuredAngle = noise ? currentState.angle + (Math.random() - 0.5) * 0.05 : currentState.angle;
			...
			const torque = Math.max(-MAX_TORQUE, Math.min(MAX_TORQUE, pTerm + dTerm + iTerm));
	...
	const getAngleFromMouse = useCallback((e: React.MouseEvent<HTMLCanvasElement> | MouseEvent) => {
	...
	const isNearTarget = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
		const canvas = canvasRef.current;
	...
	const drawMotor = useCallback(
		(ctx: CanvasRenderingContext2D, angle: number, target: number, pOut: number, dOut: number, iOut: number, velocity: number, massVal: number, mousePosLocal: { x: number; y: number } | null, isInCanvasLocal: boolean, isDraggingLocal: boolean, showD: boolean) => {
			...
			if (showD) {
				...
				if (Math.abs(velocity) > 0.01) {
					...
					ctx.lineTo(arrowEndX - headLength * Math.cos(headAngle - Math.PI / 6), arrowEndY - headLength * Math.sin(headAngle - Math.PI / 6));
					ctx.lineTo(arrowEndX - headLength * Math.cos(headAngle + Math.PI / 6), arrowEndY - headLength * Math.sin(headAngle + Math.PI / 6));
					ctx.closePath();
			...
			// Power indicator arc
			const power = Math.max(-1, Math.min(1, (pOut + dOut + iOut) / MAX_TORQUE));
			if (Math.abs(power) > 0.05) {
				...
				if (arcLength > 0.2) {
					...
					const arrowAngle = power > 0 ? endAngle + arrowAdjustment : endAngle - arrowAdjustment;
					...
					ctx.lineTo(baseX + Math.cos(perpAngle) * arrowWidth, baseY + Math.sin(perpAngle) * arrowWidth);
					ctx.lineTo(baseX - Math.cos(perpAngle) * arrowWidth, baseY - Math.sin(perpAngle) * arrowWidth);
					ctx.closePath();
			...
			if (massVal > 0) {
				...
				const massGradient = ctx.createRadialGradient(massX - massSize * 0.3, massY - massSize * 0.3, 0, massX, massY, massSize);
				...
				ctx.stroke();
				ctx.beginPath();
				ctx.arc(massX - massSize * 0.3, massY - massSize * 0.3, massSize * 0.25, 0, Math.PI * 2);
	...
	useEffect(() => {
		...
		const loop = () => {
			if (isRunning) {
				...
				currentState = simulate(currentState, targetAngle, kp, effectiveKd, effectiveKi, effectiveMass, noiseEnabled && enableNoise);
			...
			drawMotor(ctx, currentState.angle, targetAngle, pOutput, dOutput, iOutput, currentState.angularVelocity, enableMass ? mass : 0, mousePos, isInCanvas, isDragging, enableD);
			if (plotCanvasRef.current) {
				const plotCtx = plotCanvasRef.current.getContext("2d");
				if (plotCtx) drawPlot(plotCtx, historyRef.current, targetHistoryRef.current);
			}
			animationRef.current = requestAnimationFrame(loop);
		};
		animationRef.current = requestAnimationFrame(loop);
		return () => { if (animationRef.current) cancelAnimationFrame(animationRef.current); };
	}, [simulate, drawMotor, drawPlot, targetAngle, isRunning, isVisible, kp, kd, ki, mass, noiseEnabled, pOutput, dOutput, iOutput, mousePos, isInCanvas, isDragging, enableD, enableI, enableMass, enableNoise]);
		...
		<div ref={containerRef} className="not-prose flex flex-col gap-4 p-6 bg-zinc-950 w-full rounded-3xl">
			<div className="flex flex-col md:flex-row gap-4 items-start w-full">
				<div className="flex-1 flex flex-col items-center min-w-0 pb-4">
					<span className={`text-xs font-mono mb-2 transition-colors ${isInCanvas ? "text-orange-400" : "text-zinc-500"}`}>
					...
					<canvas
						...
						onMouseLeave={() => { setIsDragging(false); setIsInCanvas(false); setMousePos(null); }}
			...
			<div className="flex flex-col gap-3 px-2 pb-2">
				...
				<div className="flex items-center gap-4">
					...
					<div className="flex-1 relative h-2">
						...
						<div className="absolute left-0 top-0 h-full bg-blue-500 rounded-lg" style={{ width: `${((kp - 0.1) / 4.9) * 100}%` }} />
						<input type="range" min="0.1" max="5" step="0.1" value={kp} onChange={(e) => setKp(parseFloat(e.target.value))} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
						<div className="absolute top-1/2 -translate-y-1/2 w-4 h-4 bg-blue-500 rounded-full border-2 border-blue-300 pointer-events-none" style={{ left: `calc(${((kp - 0.1) / 4.9) * 100}% - 8px)` }} />
					</div>
					<span className="text-sm font-mono text-blue-400 w-12 text-right" style={{ fontVariantNumeric: "tabular-nums" }}>{kp.toFixed(1)}</span>
				</div>
				...
				{enableD && (
					<div className="flex items-center gap-4">
						...
						<div className="flex-1 relative h-2">
							<div className="absolute inset-0 bg-zinc-800 rounded-lg" />
							<div className="absolute left-0 top-0 h-full bg-purple-500 rounded-lg" style={{ width: `${(kd / 2) * 100}%` }} />
							<input type="range" min="0" max="2" step="0.05" value={kd} onChange={(e) => setKd(parseFloat(e.target.value))} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
							<div className="absolute top-1/2 -translate-y-1/2 w-4 h-4 bg-purple-500 rounded-full border-2 border-purple-300 pointer-events-none" style={{ left: `calc(${(kd / 2) * 100}% - 8px)` }} />
						</div>
						<span className="text-sm font-mono text-purple-400 w-12 text-right" style={{ fontVariantNumeric: "tabular-nums" }}>{kd.toFixed(2)}</span>
					</div>
				)}
				...
				{enableI && (
					<div className="flex items-center gap-4">
						...
						<div className="flex-1 relative h-2">
							<div className="absolute inset-0 bg-zinc-800 rounded-lg" />
							<div className="absolute left-0 top-0 h-full bg-green-500 rounded-lg" style={{ width: `${(ki / 1) * 100}%` }} />
							<input type="range" min="0" max="1" step="0.05" value={ki} onChange={(e) => setKi(parseFloat(e.target.value))} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
							<div className="absolute top-1/2 -translate-y-1/2 w-4 h-4 bg-green-500 rounded-full border-2 border-green-300 pointer-events-none" style={{ left: `calc(${(ki / 1) * 100}% - 8px)` }} />
						</div>
						<span className="text-sm font-mono text-green-400 w-12 text-right" style={{ fontVariantNumeric: "tabular-nums" }}>{ki.toFixed(2)}</span>
					</div>
				)}
				...
				{enableMass && (
					<div className="flex items-center gap-4">
						...
						<div className="flex-1 relative h-2">
							<div className="absolute inset-0 bg-zinc-800 rounded-lg" />
							<div className="absolute left-0 top-0 h-full bg-zinc-500 rounded-lg" style={{ width: `${mass * 100}%` }} />
							<input type="range" min="0" max="1" step="0.05" value={mass} onChange={(e) => setMass(parseFloat(e.target.value))} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" />
							<div className="absolute top-1/2 -translate-y-1/2 w-4 h-4 bg-zinc-500 rounded-full border-2 border-zinc-300 pointer-events-none" style={{ left: `calc(${mass * 100}% - 8px)` }} />
						</div>
						<span className="text-sm font-mono text-zinc-400 w-12 text-right" style={{ fontVariantNumeric: "tabular-nums" }}>{mass.toFixed(2)}</span>
					</div>
				)}
			</div>
			...
			<div className="flex justify-between items-center flex-wrap gap-3">
				<div className="flex gap-2 items-center">
					<button type="button" onClick={handleReset} className="p-2.5 bg-zinc-900 hover:bg-zinc-800 active:bg-zinc-700 text-zinc-300 rounded-xl transition-all" title="Reset">
						<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
							<path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8" /><path d="M3 3v5h5" />
						</svg>
					</button>
					<button type="button" onClick={() => setIsRunning(!isRunning)} className={`p-2.5 rounded-xl transition-all ${isRunning ? "bg-amber-600 hover:bg-amber-500 active:bg-amber-400 text-white" : "bg-emerald-600 hover:bg-emerald-500 active:bg-emerald-400 text-white"}`} title={isRunning ? "Pause" : "Play"}>
						{isRunning ? (
							<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
					...
					{enableNoise && (
						<button type="button" onClick={() => setNoiseEnabled(!noiseEnabled)} className={`p-2.5 rounded-xl transition-all ${noiseEnabled ? "bg-red-600 hover:bg-red-500 active:bg-red-400 text-white" : "bg-zinc-900 hover:bg-zinc-800 active:bg-zinc-700 text-zinc-300"}`} title={noiseEnabled ? "Noise ON" : "Noise OFF"}>
							<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
				...
				<div className="flex gap-3 font-mono text-xs">
					<div className="px-3 py-1.5 bg-zinc-900 rounded-xl flex flex-col items-center w-[70px]">
						...
						<span className="text-red-400" style={{ fontVariantNumeric: "tabular-nums" }}>
							<span className="inline-block w-[0.6em] text-right">{(error * 180) / Math.PI < 0 ? "−" : ""}</span>
					...
					{enableD && (
						<div className="px-3 py-1.5 bg-zinc-900 rounded-xl flex flex-col items-center w-[70px]">
							...
							<span className="text-purple-400" style={{ fontVariantNumeric: "tabular-nums" }}>
								<span className="inline-block w-[0.6em] text-right">{dOutput < 0 ? "−" : ""}</span>
