...
<div className="flex items-center gap-3 my-6">
  ...
  <span className="text-zinc-400 text-sm">
    <span className="text-orange-400 font-medium">Orange elements are interactive</span> — click or drag to explore
  </span>
</div>
...
The motor has no idea it should slow down as it approaches the target! It's providing full torque right up until it passes the target angle, then switching to full reverse torque. This is why the pointer oscillates; in fact, it never settles down. We need something smarter.
...
2. **Computes** the error = target − current
...
<div className="my-6 p-5 bg-zinc-900/50 border border-zinc-800 rounded-xl">
<div className="flex items-center gap-2 mb-3 text-amber-400 font-medium">
<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2v4"/><path d="m6.41 6.41 2.83 2.83"/><path d="M2 12h4"/><path d="m6.41 17.59 2.83-2.83"/><path d="M12 18v4"/><path d="m17.59 17.59-2.83-2.83"/><path d="M18 12h4"/><path d="m17.59 6.41-2.83 2.83"/></svg>
Bonus: The Noise Problem
</div>
...
<div className="flex items-center gap-2 mb-3 text-amber-400 font-medium">
<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2v4"/><path d="m6.41 6.41 2.83 2.83"/><path d="M2 12h4"/><path d="m6.41 17.59 2.83-2.83"/><path d="M12 18v4"/><path d="m17.59 17.59-2.83-2.83"/><path d="M18 12h4"/><path d="m17.59 6.41-2.83 2.83"/></svg>
Bonus: Integral Windup
</div>

The integral term can cause problems of its own. **To see windup in action:**
...
<div className="flex items-center gap-2 mb-3 text-amber-400 font-medium">
<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2v4"/><path d="m6.41 6.41 2.83 2.83"/><path d="M2 12h4"/><path d="m6.41 17.59 2.83-2.83"/><path d="M12 18v4"/><path d="m17.59 17.59-2.83-2.83"/><path d="M18 12h4"/><path d="m17.59 6.41-2.83 2.83"/></svg>
Bonus: Conditional Integration
</div>

Try enabling **Conditional I** (the button with the circle icon). With this enabled, the integral only accumulates when the temperature is within 50° of the target. This prevents the integral from winding up during the initial heating phase when the error is huge.
...
Here's the key insight. Without the outer position loop, the inner angle loop always tries to keep the pendulum at 0° (vertical). But with the outer loop, we *bias* that target slightly off-vertical to push the cart back toward center:
...
1. **Angle Control** (Kp<sub>θ</sub>, Kd<sub>θ</sub>): Fast inner loop that keeps the pendulum at whatever angle it's told
...
- **Set Kd<sub>θ</sub> = 0** to see angle oscillations grow and cause failure
...
Click **Random Target** (🎲) to see how each handles step changes. Increase the **Mass** to add a disturbance and observe:
