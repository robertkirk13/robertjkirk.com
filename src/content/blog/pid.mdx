...
The $error$ is the difference between the target and current position, and $K_p$ is the proportional gain that determines how aggressive the response is.
...
Play with the $K_p$ slider and target angle. Notice that there's a tradeoff:
...
Like magic, the oscillation is almost entirely gone! The system now settles smoothly to the target instead of bouncing back and forth. Try setting $K_d$ to 0 to see the difference: the oscillation returns immediately.
...
This is **steady-state error**. Proportional control only pushes when there's error. To counteract the constant pull of gravity, some error must remain — otherwise there's nothing to push against. Higher $K_p$ shrinks the gap but can't close it.
