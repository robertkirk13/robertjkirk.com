...
<PIDDemo client:load enableD={true} enableI={true} enableMass={true} enableNoise={true} initialKp={2.0} initialKd={0.8} initialKi={0.3} initialMass={0.5} />
...
But there's a catch: the integral term can cause problems of its own. When there's a large error for a long time (like during startup), the integral accumulates a huge value. Even after reaching the target, this "wound up" integral causes overshoot and slow settling. This is called **integral windup**.

The demo above uses **clamping** to prevent windup&mdash;the integral is limited to a maximum value (±10 in this case). Other strategies include:
...
Each approach has tradeoffs, but clamping is the most common starting point.
...
3. **Add I**: Increase Ki to eliminate any remaining steady-state error (use sparingly!)
...
Finally, here's a playground where you can compare different control strategies side-by-side. Watch how No Control, P-only, PD, and full PID respond to the same target:
...
- **P-only** gets close but oscillates
- **PD** settles quickly but may have steady-state error with disturbances
- **PID** balances fast response with minimal overshoot and no steady-state error
